...
OFMessageHandler::Action MyApp::Handler::BGPprocess(OFConnection* ofconn, Flow* flow)
{
	std::cout << "BGPprocess start" << std::endl;
	...
	std::cout << "BGPprocess start TCP/IP" << std::endl;
	// check TCP port == BGP port (179)
	// TODO: how to compare with tcp_port=179???
	//if ( !(of13::TCPDst(179).equals(oxm_fields)) )
	//	return OFMessageHandler::Continue;
	std::cout << "BGPprocess TCPDst == 179" << std::endl;
	...
	// Create tcp_dst oxf_field with value=179 (bgp) and add to Flow Mod
	fm.add_oxm_field(new of13::TCPDst(179));

	fm.add_oxm_field(new of13::IPv4Src(packet->readIPv4Src()));
	fm.add_oxm_field(new of13::IPv4Dst(packet->readIPv4Dst()));

	// Creates Output Action with in_port=5(veth_of1) and send_len=1024
	// of13::OutputAction act(1, 1024);
	of13::OutputAction act(5, 1024);
	...
	of13::FlowMod fm_ret;
	fm_ret.add_oxm_field(new of13::InPort(5));
	fm_ret.add_oxm_field(new of13::EthType(0x0800));
	fm_ret.add_oxm_field(new of13::IPProto(6));
	fm_ret.add_oxm_field(new of13::TCPDst(179));
	fm_ret.add_oxm_field(new of13::IPv4Src(packet->readIPv4Dst()));
	fm_ret.add_oxm_field(new of13::IPv4Dst(packet->readIPv4Src()));
	//of13::OutputAction act_ret(, 1024);
	
	
	return OFMessageHandler::Continue;
}

OFMessageHandler::Action MyApp::Handler::ARPprocess(OFConnection* ofconn, Flow* flow)
{
	
	// if !ARP then exit
	if (flow->pkt()->readEthType() != 0x0806)
		return Continue;
	if (flow->pkt()->readARPOp() == 1)
	{
		uint8_t size_mes = (64/8) + 2*(48/8) + 2*(32/8);
		uint8_t * mes = reinterpret_cast<uint8_t*>( new char[size_mes] );
		*(reinterpret_cast<uint16_t*>(mes)) = 0x0001;
		*(reinterpret_cast<uint16_t*>(mes+2)) = 0x0800;
		*(reinterpret_cast<uint8_t*>(mes+4)) = 6;
		*(reinterpret_cast<uint8_t*>(mes+5)) = 4;
		*(reinterpret_cast<uint16_t*>(mes+6)) = 2;
		// Addresses...
		// eth1 == 100.0.1.1 == 52:54:00:aa:3c:c5
		// eth2 == 100.0.4.1 == 52:54:00:6d:da:9d
		if (flow->pkt()->readARPTPA() == IPAddress("100.0.4.1")) // eth2
		{
			*(reinterpret_cast<uint32_t*>(mes+8)) = 0x5254006d;
			*(reinterpret_cast<uint16_t*>(mes+12)) = 0xda9d;
			*(reinterpret_cast<uint16_t*>(mes+14)) = 100*256 + 0;
			*(reinterpret_cast<uint16_t*>(mes+16)) = 4*256 + 1;
		}
		else if (flow->pkt()->readARPTPA() == IPAddress("100.0.1.1")) // eth1
		{
			*(reinterpret_cast<uint32_t*>(mes+8)) = 0x525400aa;
			*(reinterpret_cast<uint16_t*>(mes+12)) = 0x3cc5;
			*(reinterpret_cast<uint16_t*>(mes+14)) = 100*256 + 0;
			*(reinterpret_cast<uint16_t*>(mes+16)) = 1*256 + 1;
		}
		else
		{
			delete[] mes;
		...
		uint8_t * sha = flow->pkt()->readARPSHA().get_data();
		*(reinterpret_cast<uint16_t*>(mes+18)) = *(reinterpret_cast<uint8_t*>(sha));
		*(reinterpret_cast<uint32_t*>(mes+20)) = *(reinterpret_cast<uint32_t*>(sha+2));
		*(reinterpret_cast<uint32_t*>(mes+24)) = flow->pkt()->readARPSPA().getIPv4();
		ofconn->send(mes, size_mes);
		delete[] mes;
	}
	return Continue;
}
...
{
	int eth_type = flow->pkt()->readEthType();
	printf("EthType == 0x%x\n", eth_type);
	/*
	if (eth_type == 0x0806)// || (eth_type == 0x0800 && flow->pkt()->read)
	{
		of13::FlowMod fm;
		// Add in_port to Flow Mod
		fm.add_oxm_field(new of13::InPort(packet->readInPort()));
		...
		ofconn->send(buff, fm.length());
		delete[] buff;
		//std::cout << "ARP : type == " << flow->pkt()->readARPOp() << std::endl;
		//std::cout << flow->pkt()->readARPSHA() << " ---> " << flow->pkt()->readARPTHA() << std::endl;
		//std::cout << flow->pkt()->readARPSPA() << " ---> " << flow->pkt()->readARPTPA() << std::endl;
	}
	*/
	OFMessageHandler::Action value;
	if (BGPprocess(ofconn, flow) == Stop)
		return Stop;
